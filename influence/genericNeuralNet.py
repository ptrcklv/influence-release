...
class GenericNeuralNet(object):
    ...
    def get_vec_to_list_fn(self):
        """Return a function that transforms a vector to a list decorated with object specific values
        Args: None
        Returns: vec_to_list function
        ...
        def vec_to_list(v):
            """Takes a vector v which consists of all parameters and puts them into a
            list. A single parameter can be a vector of arbitrary length.
            The length of each parameter vector is taken from a global variable params_val,
            which corresponds to the params of the containing GenericNeuralNet object.
            The idea is like this, v contains all parameters in the form
            v= [p11, ..., p1n, p21, ..., p2m, p31, ...] where p1 is the first parameter of length n,
            p2 is the second parameter of length m, ...
            The return list is the a list of lists, where each internal list corresponds to
            one parameter veector:
            return_list = [[p1, ..., p1n], [p21, ..., p2m], ...]

            Args:
                - v: parameter vector
            Returns:
                - return_list: the values of v in a two-dimensional list
            """
    ...
    def fill_feed_dict_with_all_ex(self, data_set):
        ...
        Args:
            - data_set: must have an attribute "x" that contains the features and
            an attribute "labels" that contains the corresponding labels.
        Returns:
            - dictionary associating the class placeholders with the features and labels, respectively.
        """
    ...
    def fill_feed_dict_with_all_but_one_ex(self, data_set, idx_to_remove):
        """Removes one row from the dataset specified by row number. Provides
        the resulting data set as feed dict.
        Args:
            data_set: Data set to remove one item from
            idx_to_remove: row number to remove
        Returns:
            feed_dict: Feed dict with the data set with the one row removed
        """
